...
import { Text, Platform, View, Dimensions, Modal, Animated, ScrollView } from 'react-native';
...
class ActionSheet extends Component {
  ...
  calculateHeight(props) {
    const { options, buttonHeight, titleHeight, messageHeight, cancelMargin, isIphoneX } = props;

    this.iPhoneXOffset = isIphoneX ? 22 : 0;

    let height = options.reduce((sum, { height: optionHeight = buttonHeight }) => (sum += optionHeight), cancelMargin);
  ...
  renderOptions(styles) {
    ...
    return options.map((option, index) => {
      ...
      const disable = (option && option.disable) || false;
  ...
  render() {
    const {
      props: { styles: userStyles, message, messageHeight, title, titleHeight },
      state: { visible, sheetPositionY },
    } = this;
    const styles = Object.assign({}, userStyles, defaultStyles);

    return (
      <Modal visible={visible} transparent={true} animationType="none" onRequestClose={this.cancel}>
        <View style={[styles.wrapper, { paddingBottom: this.iPhoneXOffset }]}>
          <Animated.Text
            style={[
              styles.overlay,
              {
                opacity: sheetPositionY.interpolate({
                  outputRange: [0.4, 0.4, 0, 0],
                  inputRange: [-1, 0, this.translateY, this.translateY+1],
                }),
              },
            ]}
            onPress={this.cancel} />
          ...
          >
            {title && (
              <Message text={title} titleStyle={styles.titleText} style={[styles.title, { height: titleHeight }]} />
            )}
